...
try {
    ...
    // Core query matching logs to correct profiles, handling empty person_type strings automatically
    $query = "SELECT 
                al.id,
                al.person_type,
                al.log_type,
                al.timestamp,
                COALESCE(r.full_name, fp.full_name) AS full_name,
                CASE 
                    WHEN r.id IS NOT NULL THEN 'Resident'
                    WHEN fp.id IS NOT NULL THEN fp.role_type
                    ELSE IF(al.person_type = '', 'Personnel', al.person_type)
                END AS display_role,
                COALESCE(r.registered_vehicle_plate, fp.vehicle_plate) AS vehicle_plate
              FROM access_logs al
              LEFT JOIN residents r ON al.person_id = r.id AND (LOWER(al.person_type) = 'resident' OR al.person_type = '')
              LEFT JOIN frequent_personnel fp ON al.person_id = fp.id AND (LOWER(al.person_type) != 'resident' OR al.person_type = '')
    ...
    $logs = [];
    $error_msg = "System Log Stream Error: " . $e->getMessage();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>ResiCured Admin - Gate Tracking Logs</title>
...
<div class="page-wrapper">
    ...
    <div class="main-content">
        <div class="d-flex justify-content-between align-items-center pb-3 mb-4 border-bottom">
            <div>
                <h1 class="h3 fw-bold text-dark m-0">Perimeter Access Log Archive</h1>
                <p class="text-muted small mb-0">Review comprehensive historical records of security check transactions captured across terminals.</p>
            </div>
            <button onclick="window.location.reload();" class="btn btn-sm btn-light border fw-semibold text-secondary">
                <i class="fa-solid fa-sync me-1"></i> Refresh Log Stream
            </button>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if ($l_type === 'entry' || $l_type === 'timein' || $l_type === 'time in'): ?>
                                            <span class="badge badge-timein px-3 py-1.5 text-uppercase"><i class="fa-solid fa-right-to-bracket me-1"></i> Time In</span>
                                        <?php else: ?>
                                            <span class="badge badge-timeout px-3 py-1.5 text-uppercase"><i class="fa-solid fa-right-from-bracket me-1"></i> Time Out</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
